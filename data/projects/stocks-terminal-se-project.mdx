export const name = "StocksTerminal";
export const image = "/images/projects/StockTerminal.png";
export const overview = "CLI app to retrieve and analyze real-time stock data: query by symbol and flags, live feed, and history (Software Engineering project).";
export const topics = ["Java", "Fintech", "Stock Data", "CLI"];
export const repoUrl = "https://github.com/MaxFdev/StocksTerminal_SE_Project";

## Overview

StocksTerminal is a Java command-line application that retrieves and displays stock market data; it is typically run as a JAR. The app calls an external stock/market data API to fetch quotes and uses in-memory caching for recent symbols. You run the program (e.g. via the JAR or the project entry point) and use “stock” as the first argument; then you issue commands to query one or more ticker symbols, request specific data fields, refresh or clear cache, list history, or run a live updating feed for a symbol. It is aimed at quick lookups (e.g. by brokers or developers) and can be scripted (e.g. bash calling the app and reacting to output).
...
- The **CLI** parses commands (help, query, clear, refresh, remove, history, Live, quit) and either looks up **cached** data or calls the **stock API** (or backend) for fresh data.
...
- **Query**: `[stock_symbol(s)] [request_flags]`  
  Fetches data for the given symbol(s). Symbols and flags are separated by spaces. Example: `aapl amzn -pov` gets price, open, and volume for AAPL and AMZN.  
  If you don’t specify flags, the app assumes all fields (equivalent to something like `-cehloprtv`).
...
So `-pov` means previous close, open, and volume; no flags often means “all of the above.”
...
Run the program (e.g. JAR or from the `stockterminal_se` project). First argument is typically “stock”; then use the commands above. Example query:
