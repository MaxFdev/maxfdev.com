export const company = "Yeshiva University";
export const role = "Software Engineering Intern";
export const dates = "June 2025 - Present";
export const image = "/images/experience/yu.png";
export const overview =
  "Developed The Rav Project, an online learning platform, with TypeScript, Next.js, Supabase, Sanity CMS, and a custom RAG AI system. Built APIs, optimized SQL, and implemented CI/CD.";
export const companyUrl = "https://theravlegacy.org";

## Overview

The Rav Project (or Rav Legacy Project) is an online learning platform built around The Rav's lectures. I worked on a teaming using TypeScript, and Next.js (React) to build the platform, Supabase (PostgreSQL) for user data, Sanity CMS (NoSQL) for site content, and a custom RAG (Retrieval-Augmented Generation) AI for semantic search over the lectures. As an intern I worked across the stack. I built a REST API for the RAG pipeline, wrote and optimized SQL for security and user management, built a TypeScript client for Sanity, and set up CI/CD for deployments on Vercel.

## System Architecture

The platform has four main pieces: **content** in Sanity CMS, **user data** in Supabase, **the site** in Next.js (lectures and UI), and **smart search** via the RAG AI over embedded lecture text.

```mermaid
flowchart TB
    subgraph content ["Content (Sanity CMS)"]
        Sanity["Sanity CMS"]
        Pages["Pages & metadata"]
        Media["Media references"]
        Sanity --> Pages
        Sanity --> Media
    end

    subgraph userdata ["User data (Supabase)"]
        Supabase["Supabase · PostgreSQL"]
        Auth["Auth & profiles"]
        Permissions["Permissions"]
        Supabase --> Auth
        Supabase --> Permissions
    end

    subgraph site ["Next.js site"]
        App["Next.js app"]
        Lectures["Lecture display"]
        SearchUI["Search UI"]
        App --> Lectures
        App --> SearchUI
    end

    subgraph rag ["RAG AI"]
        RAG["RAG system"]
        Embeddings["Embeddings"]
        VectorStore["Vector store"]
        RAG --> Embeddings
        RAG --> VectorStore
    end

    content --> site
    userdata --> site
    site --> rag
    rag --> site
```

- **Sanity CMS.** Holds the editorial content (pages, navigation, media). I built a TypeScript client so the backend can read and write this content.
- **Supabase.** User accounts, profiles, and permissions. We use SQL with row-level security, as well as custom RPC functions.
- **Next.js site.** It pulls content from Sanity and user data from Supabase, renders the lectures and search UI, and talks to the RAG system when you run a query.
- **RAG AI.** It takes lecture text from the pipeline below, turns it into embeddings, and stores them in a vector store so we can do semantic search.

## Lecture Data Pipeline

Lecture material goes from **audio** to **transcription** to **storage** to **RAG embedding** so you can search it by meaning.

```mermaid
flowchart LR
    subgraph ingest ["Ingest"]
        Audio["Audio recordings"]
        Transcribe["Transcription"]
        Audio --> Transcribe
    end

    subgraph upload ["Upload & store"]
        Studio["Site upload studio"]
        Sanity["Sanity CMS"]
        Transcribe --> Studio
        Studio --> Sanity
    end

    subgraph expose ["Expose"]
        API["REST API"]
        Sanity --> API
    end

    subgraph rag_pipeline ["RAG pipeline"]
        Expose["Expose documents"]
        Embed["Embedding service"]
        Vectors["Vector store"]
        Expose --> Embed
        Embed --> Vectors
    end

    API -->|"Document chunks"| Expose
```

- **Audio and transcription.** Lectures get transcribed (speech-to-text). That text feeds the rest of the pipeline.
- **Store and expose.** Transcribed content lives in the Sanity. A **REST API** I built exposes that lecture data as document chunks for the RAG pipeline.
- **RAG embedding.** The pipeline takes that data, chunks it, runs it through an embedding model, and writes vectors to the store. Smart search runs over that store.

## Key Contributions

- **REST API for RAG.** I built an API that exposes lecture data (transcripts and metadata) for the embedding pipeline.
- **SQL and security on Supabase.** I wrote and optimized SQL, implemented row-level security, and set up user management.
- **CI/CD on Vercel.** I set up a pipeline to automate build, test, and deploy so branches get deployed as previews or to production.
